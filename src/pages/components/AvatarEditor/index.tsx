...
export default function AvatarEditor() {
    ...
    const generatePreview = async (flipped: boolean) => {
        const groups = await Promise.all(
            Object.keys(AvatarStyleCount).map(async (type) => {
                /* eslint-disable */
                const svgRaw = (
                    await require(`!!raw-loader!@/public/avatar/preview/${type}/${config[type as AvatarPart]
                        }.svg`)
                ).default
                return `\n<g id="notion-avatar-${type}" ${type === 'face' ? 'fill="#ffffff"' : ''
    ...
    \n</g>\n`
        ...
        if (festival) {
            ...
            groups.push(`\n<g id="notion-avatar-${festival}" ${flipped ? 'transform="scale(-1,1) translate(-1080, 0)"' : ''
                }>\n
    ${svgRaw.replace(/<svg.*(?=>)>/, '').replace('</svg>', '')}
  \n</g>\n`)
        }
    ...
    return (
        ...
        />
            ...
            <div className='text-center my-12 max-w-3xl mx-auto'>
                <h1 className='text-5xl font-bold mb-6'>Notion Faces</h1>
                <p className='text-xl mb-6'>Create beautiful minimalist Notion style avatars for your needs. </p>
            </div>
            <div className="flex justify-center items-center flex-col">
                <div
                    ...
                    id="avatar-preview"
                    className={`w-48 h-48 md:w-72 md:h-72 ${ShapeStyleMapping[background.shape]
                        }`}
                    ...
                    <div className="grid gap-y-4 justify-items-center justify-between grid-rows-2 grid-cols-5 lg:flex">
                        {Object.keys(AvatarStyleCount).map((type) => (
                            <div key={type} className="relative" id={`avatar-picker-${type}`}>
                                ...
                                >
                                    <Image
                                        src={`/avatar/part/${type}/${type}-${config[type as AvatarPart]
                                            }.svg`}
                                        width={30}
                                        height={30}
                                    />
